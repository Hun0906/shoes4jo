...
public interface GoodsTrendMapper {
    ...
    @Select("SELECT * FROM `4jo_api_search_device` WHERE keyword = #{keyword} and device = #{device} ORDER BY period_sdata DESC limit 30")
    ...
    @Select("SELECT count(*) FROM 4jo_api_search_device WHERE period_sdata = #{period_sdata} and keyword = #{keyword} and device = #{device}")
    ...
    @Select("SELECT ratio_cnt FROM 4jo_api_search_device WHERE period_sdata = #{period_sdata} and keyword = #{keyword} and device = #{device}")
    ...
    @Insert("INSERT INTO 4jo_api_search_device (period_sdata, keyword, device, ratio_cnt) VALUES (#{period_sdata}, #{keyword}, #{device}, #{ratio_cnt})")
    ...
    @Update("UPDATE 4jo_api_search_device SET ratio_cnt = #{ratio_cnt} WHERE period_sdata = #{period_sdata} and keyword = #{keyword} and device = #{device}")
