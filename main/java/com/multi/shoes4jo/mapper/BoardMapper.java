...
public interface BoardMapper {
	...
	@Select("SELECT bno, category, title, content, writer, viewcnt, regdate FROM 4jo_board ORDER BY bno DESC LIMIT #{perPageNum} OFFSET #{pageStart}")
	public List<BoardVO> listPage(Criteria cri);
	// 페이징 목록조회

	@Select("SELECT COUNT(bno) FROM 4jo_board WHERE bno > 0")
