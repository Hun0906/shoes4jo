 ...
 * response) throws ServletException, IOException { response.sendRedirect("/");
 * }
 * 
 ...
 * 
 * if (loginRes == 1) { HttpSession session = request.getSession();
 * session.setAttribute("id", id);
 * 
 * session.setMaxInactiveInterval(30 * 60);
 * 
 * response.sendRedirect("/"); } else { RequestDispatcher dispatch =
 * request.getRequestDispatcher("login.jsp?error="+loginRes);
